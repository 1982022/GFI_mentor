...
file1 = open("GFI_mentor/GFI_selection/RQ1/commit_commenter.csv", "r")
csv_reader = csv.reader(file1)
next(csv_reader)

file2 = open("GFI_mentor/GFI_selection/RQ1/commit_commenter_exp_distribution.csv", "w")
...
for row in csv_reader:
    ...
    if row[0] != issue_id:
        ...
        if row[5] != '':
            num_comment += 1
            commenters.append(row[4])
        if row[4] == 'newcomers':
            newcomers.append(row[4])
            num_newcomer_comment += 1
        if row[4] == 'expert':            
            experts.append(row[4])
            num_expert_comment += 1
res.append([issue_id, created_at, num_comment, len(set(commenters)), len(set(newcomers)), len(set(experts)), num_newcomer_comment, num_expert_comment]) 
res = res[1:]
...
plt.show()
plt.savefig("commit_comments_distribution.png")
...
plt.show()
plt.savefig("commit_commenters_distribution.png")
